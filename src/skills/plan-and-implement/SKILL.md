...
1. Read `src/skills/_shared/references/context-summary.md`.
...
- **No code-line review comment may be left unaddressed.** Before marking any subtask complete, fetch all PR review comments on code lines. For each comment from a reviewer: either implement the suggested change, or reply with an explanation of why the current code is correct. After all comments are handled, verify with `npx tsx scripts/verify-no-unresolved-comments.ts`. The final state must have zero unresolved code-line review comments.
